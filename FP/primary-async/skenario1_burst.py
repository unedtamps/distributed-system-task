...
def run_scenario():
    ...
    for i in range(1, total_rows + 1):
        ...
        end_write = time.time()
        if i % 100 == 0 or i == 1:
            print(f"INSERT {i} rows")

    future_r1 = executor.submit(
        check_replica, cur_r1, conn_r1, "Replica_1", total_rows, end_write
    )
    future_r2 = executor.submit(
        check_replica, cur_r2, conn_r2, "Replica_2", total_rows, end_write
    )
    ...
    print("\nHasil Observasi")
    print(f"Lag Replica 1: {min(lags_r1):.6f} detik")
    print(f"Lag Replica 2: {min(lags_r2):.6f} detik")
